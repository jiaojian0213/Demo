 ...
 */
public class SketchGraphicsOverlay implements Observer {

    private final MapView mMapView;
    private final GraphicsLayer mGraphicsOverlay;
    private final GraphicList mGraphics;
...
//    private GeometryInfo geometryInfo;
    ...
    public SketchGraphicsOverlay(MapView mapView, SketchGraphicsOverlayEventListener listener) {
        mMapView = mapView;
        mListener = listener;
        mGraphicsOverlay = new GraphicsLayer();
        graphicIdMap = new ArrayMap<>();
        // Add a graphics overlay and get our list of graphics for modification
        mMapView.addLayer(mGraphicsOverlay);
        mGraphics = new GraphicList(this);
...
//        mMapView.setOnTouchListener(new DrawingMapViewOnTouchListener(mMapView.getContext(), mMapView));
        ...
        SimpleLineSymbol blackOutline =
                new SimpleLineSymbol(Color.rgb(0, 0, 0), 1, SimpleLineSymbol.STYLE.SOLID);
        SimpleLineSymbol whiteOutline =
                new SimpleLineSymbol(Color.rgb(255, 255, 255), 1, SimpleLineSymbol.STYLE.SOLID);

        // Create all the different symbols
        // When placing a point, it will be a red circle with black outline
        mPointPlacementSymbol =
                new SimpleMarkerSymbol(Color.RED, 17, SimpleMarkerSymbol.STYLE.CIRCLE);
        mPointPlacementSymbol.setOutline(blackOutline);
        // A placed point (single point) will be a blue circle with black outline
        mPointPlacedSymbol =
                new SimpleMarkerSymbol(Color.BLUE, 17, SimpleMarkerSymbol.STYLE.CIRCLE);
        ...
        if (!from.isEmpty()) {
            ...
            switch (item.getEvent()) {
                ...
                case ADD_POLYLINE_POINT:
                    if(mCurrentPointCollection.size() == 0){
                        mGraphics.clear();
                        mGraphicsOverlay.removeAll();
                        break;
                    }
                    ...
                    setField(mCurrentLine, "geometry", getPolyline(mCurrentPointCollection));
//          mCurrentLine.setGeometry(getPolyline(mCurrentPointCollection));
                    if (mDrawingMode == DrawingMode.POLYGON) {
                        setField(mCurrentPolygon, "geometry", getPolygon(mCurrentPointCollection));
                        showAreaGraphic();
//            mCurrentPolygon.setGeometry(getPolyline(mCurrentPointCollection));
                    }
...
//          mCurrentPoint.setSymbol(mPointPlacementSymbol);
                    setField(mCurrentPoint, "symbol", mPointPlacementSymbol);
...
//          mCurrentPoint.setSymbol(mPolylineVertexSymbol);
                    setField(mCurrentPoint, "symbol", mPolylineVertexSymbol);
...
//            mCurrentPoint.setGeometry(newGeometry);
                        setField(mCurrentPoint, "geometry", newGeometry);
                    } else {
...
//            mCurrentPoint.setGeometry(oldPoint);
                        setField(mCurrentPoint, "geometry", oldPoint);
...
//              mGraphics.get(index - 1).setGeometry(preMidpoint);
                            setField(mGraphics.get(index - 1), "geometry", preMidpoint);
                        }
...
//              mGraphics.get(index + 1).setGeometry(postMidpoint);
                            setField(mGraphics.get(index + 1), "geometry", postMidpoint);
                        }
                    }
                    if (mDrawingMode == DrawingMode.POLYGON) {
...
//            mCurrentPoint.setGeometry((Geometry) item.getElement());
                        setField(mCurrentPoint, "geometry", item.getElement());
                    }
                    break;
...
//    mCurrentLine.setGeometry(new Polyline(mCurrentPointCollection));
        ...
        Logger.i("current point = " + mCurrentPoint);
        if (mDrawingMode == DrawingMode.POLYGON) {
//      mCurrentPolygon.setGeometry(new Polygon(mCurrentPointCollection));
    ...
    public void showAreaGraphic(){
        mMapView.postDelayed(new Runnable() {
            @Override
            public void run() {
                String areaResultStr = WKTUtils.formatArea(WKTUtils.calculateArea2D(mCurrentPolygon.getGeometry()));
                PictureMarkerSymbol textSymbol = MarkerUtils.createTextPicMarkerSymbol(
                        areaResultStr, 16, Color.BLACK, Color.RED,
                        Color.argb(100, 255, 255, 255));//红框、黑字
                if(areaGraphic == null) {
                    areaGraphic = new Graphic(mCurrentPointCollection.get(0), textSymbol);
                    int id = mGraphicsOverlay.addGraphic(areaGraphic);
                    graphicIdMap.put(areaGraphic,id);
                }else{
//                    Logger.i("area = "+areaResultStr);
                    setField(areaGraphic, "symbol", textSymbol);
                }
            }
        },50);
    }
...
//      postMidpoint.setGeometry(postMidpointGeometry);
            setField(postMidpoint, "geometry", postMidpointGeometry);
        }
    }
...
//          mCurrentPoint.setSymbol(mPointPlacedSymbol);
                    setField(mCurrentPoint, "symbol", mPointPlacedSymbol);
                    mCurrentPoint = null;
...
//          mCurrentLine.setSymbol(mPolylinePlacedSymbol);
                    ...
                    setField(mCurrentPoint, "symbol", mPolylineVertexSymbol);
                    setField(mCurrentLine, "symbol", mPolylinePlacedSymbol);
    ...
    public void update(Observable o, Object arg) {
        if (arg instanceof GraphicList.GraphicEvent) {
            GraphicList.GraphicEvent graphicEvent = (GraphicList.GraphicEvent) arg;
            if (graphicEvent.type == GraphicList.EventType.ADD) {
                int id = mGraphicsOverlay.addGraphic(graphicEvent.graphic);
                graphicIdMap.put(graphicEvent.graphic, id);
//                Logger.i("update mGraphicsOverlay" + mGraphicsOverlay.getGraphicIDs().length);
//                Logger.i("update graphicIdMap " + graphicIdMap.size());
            } else if (graphicEvent.type == GraphicList.EventType.REMOVE) {
                mGraphicsOverlay.removeGraphic(graphicIdMap.get(graphicEvent.graphic));
                graphicIdMap.remove(graphicEvent.graphic);
            } else {
//                Logger.i("else event type");
            }
        }
    }
     ...
     */
    private class DrawingMapViewOnTouchListener extends MapOnTouchListener {
...
//            IdentifyGraphicsOverlayResult identifyResult = identifyGraphic.get();
                List<Graphic> graphic = new ArrayList<>();//identifyResult.getGraphics();

                int[] graphicIds = mGraphicsOverlay.getGraphicIDs(event.getX(), event.getY(), 30);
                for (int i = 0; graphicIds != null && i < graphicIds.length; i++) {
                    Graphic graph = mGraphicsOverlay.getGraphic(graphicIds[i]);
//                    Logger.i("graph = "+ graph.getSymbol().toJson());  mGraphics.indexOf(graph) > 0
                    if(!(graph.getSymbol() instanceof PictureMarkerSymbol)) {
                        graphic.add(graph);
                    }
//                    Logger.i("graph indexOf= "+ mGraphics.indexOf(graph));
                }
...
//                            Logger.i("current point 1 = " + mCurrentPoint);
...
//                    mCurrentPoint.setSymbol(mPolylineMidpointSymbol);
                                setField(mCurrentPoint, "symbol", mPolylineMidpointSymbol);
//                                Logger.i("add 1");
                            } else {
...
//                            Logger.i("graph 11= "+ mCurrentPoint.getSymbol().toJson());
...
//                            // Set our current working point to the selected graphic and change its symbol to the placing symbol
                            int uid = g.getUid();
                            if(graphicIdMap.containsValue(uid)){
                                int size = graphicIdMap.size();
                                for(int i = 0;i < size;i++){
                                    Integer integer = graphicIdMap.valueAt(i);
                                    if(integer != null && integer.intValue() == uid){
                                        mCurrentPoint = graphicIdMap.keyAt(i);
                                        break;
                                    }
                                }
                                setField(mCurrentPoint, "symbol", mPointPlacementSymbol);
...
//                                mCurrentPoint = g;
//                                setField(mCurrentPoint, "symbol", mPointPlacementSymbol);
//                                int indexOf = mGraphics.indexOf(mCurrentPoint);
//                                mGraphics.remove(indexOf);
//                                mGraphics.add(indexOf,mCurrentPoint);
                            }
...
//        android.graphics.Point currentPoint = mMapView.locationToScreen((Point) mCurrentPoint.getGeometry());
                    Point screenPoint = mMapView.toScreenPoint((Point) mCurrentPoint.getGeometry());
                    android.graphics.Point currentPoint = new android.graphics.Point((int) screenPoint.getX(), (int) screenPoint.getY());//mMapView.locationToScreen((Point) mCurrentPoint.getGeometry());
...
//          android.graphics.Point toPoint = new android.graphics.Point((int) to.getX(), (int) to.getY());
                        ...
                        Point oldPointCopy = new Point(oldGeometry.getX(), oldGeometry.getY());
//          Point newGeometry = mMapView.screenToLocation(toPoint);
                        Point newGeometry = mMapView.toMapPoint(to.getX(), to.getY());
                        ...
                        if (mDrawingMode == DrawingMode.POLYLINE || mDrawingMode == DrawingMode.POLYGON) {
                            ...
                            if (mIsMidpointSelected && !mVertexDragStarted) {
                                ...
                                if (pointIndex < 0)
                                    return true;
                                Logger.i("mCurrentPointCollection size =" + mCurrentPointCollection.size() + "  pointIndex = " + pointIndex);
...
//                preMidpoint.setGeometry(preMidpointGeometry);
                                    setField(preMidpoint, "geometry", preMidpointGeometry);
                                }
...
//    }
    ...
    private Geometry getPolyline(PointCollection pointCollection) {
        return pointCollection.getLastPolyline();
    }

    private Geometry getPolygon(PointCollection pointCollection) {
        return pointCollection.getLastPolygon();
    }

    public void setField(Object obj, String field, Object value) {
        try {
            Field geometryField = obj.getClass().getDeclaredField(field);
            geometryField.setAccessible(true);
            geometryField.set(obj, value);
            if (obj instanceof Graphic) {
                Integer integer = graphicIdMap.get(obj);
                if (integer != null) {
                    mGraphicsOverlay.updateGraphic(integer, (Graphic) obj);
                }else{
                    mGraphicsOverlay.updateGraphic(((Graphic)obj).getUid(), (Graphic) obj);
                }
            }
...
//        try {
//            String polygonString = getPolygonString();
//            double area = WKTUtils.calculateArea2D(polygonString);
//            Point centerPoint = WKTUtils.getCenterPoint(polygonString);
//            geometryInfo.setPoints(polygonString);
//            geometryInfo.setCenter_x(centerPoint.getX() + "");
//            geometryInfo.setCenter_y(centerPoint.getY() + "");
//            geometryInfo.setArea(area);
//            geometryInfo.setInsurance_count(area);
//            return geometryInfo;
...
//    }
    ...
    public String getPolygonString(){
        try {
            String str = GeometryEngine.geometryToJson(mMapView.getSpatialReference(), mCurrentPolygon.getGeometry());

            String points = str.substring(str.indexOf("[[[") + 3, str.indexOf("]]]"));
            points = points.replace("],[", ";");
            points = points.substring(0,points.lastIndexOf(";"));
            Logger.i("PolygonString = " + points);
            mIsPolylineStarted = false;
            mCurrentPoint = null;
            mCurrentLine = null;
            mCurrentPolygon = null;
    ...
    public void handAddPoint(Point point){
        //如果是通过编辑进入的，不允许添加新点
...
//                  mCurrentPoint.setGeometry(point);
                setField(mCurrentPoint, "geometry", point);
            ...
            if (!mIsPolylineStarted) {
                ...
                mCurrentLine = new Graphic(getPolyline(mCurrentPointCollection), mPolylinePlacementSymbol);
                mCurrentPoint = new Graphic(point, mPointPlacementSymbol);
                //
                List<Graphic> graphics = new ArrayList<>();
                // If we're drawing a polygon, also create a polygon graphic
                if (mDrawingMode == DrawingMode.POLYGON) {
                    mCurrentPolygon = new Graphic(getPolygon(mCurrentPointCollection), mPolygonFillSymbol);
...
//        setPoints(geometryInfo.getPoints());
//    }

    public void setPoints(String points) {
        this.points = points;
        List<Point> points1 = WKTUtils.getPoints(points);
        for(int i = 0;i< points1.size();i++){
            if(i != points1.size()-1) {
                handAddPoint(points1.get(i));
            }
        }
